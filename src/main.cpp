...
static CCNode* g_selectedNode = nullptr;
static bool g_reachedSelectedNode = false;

void generateTree(CCNode* node, unsigned int index = 0) {
    ...
    if (isOpen) {
        ...
        if (children) {
            for (unsigned int i = 0; i < children->count(); ++i) {
                ...
                generateTree(static_cast<CCNode*>(child), i);
            }
        }
        ImGui::TreePop();
    }
...
void renderNodeProperties(CCNode* node) {
    ...
    if (ImGui::Button("Copy")) {
        ...
        setClipboardText(stream.str());
    }
    if (node->getUserData())
        ImGui::Text("User data: 0x%p", node->getUserData());

    const auto menuItemNode = dynamic_cast<CCMenuItem*>(node);
    if (menuItemNode) {
...
#define GET_SET_FLOAT2(name, label) { \
    ...
    ImGui::DragFloat2(label, reinterpret_cast<float*>(&point)); \
    if (point != node->get##name()) \
        node->set##name(point); \
}
...
#undef dragXYBothIDK
    ...
    const auto rgbaNode = dynamic_cast<CCNodeRGBA*>(node);
    if (rgbaNode) {
        ...
        if (ImGui::ColorEdit4("Color", colorValues)) {
            ...
            rgbaNode->setOpacity(static_cast<GLubyte>(colorValues[3] * 255.f));
        }
    }
    const auto labelNode = dynamic_cast<CCLabelProtocol*>(node);
    if (labelNode) {
...
void draw() {
    ...
    if (g_showWindow) {
        if (ImGui::Begin("cocos2d explorer", nullptr, ImGuiWindowFlags_HorizontalScrollbar)) {
            ...
            g_reachedSelectedNode = false;
            generateTree(node);
            
            ImGui::EndChild();

            if (!g_reachedSelectedNode)
            ...
            if (g_selectedNode)
                renderNodeProperties(g_selectedNode);
            else
